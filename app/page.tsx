export default function TinycopicStudioSite() {
  ...
  // 部署说明：
  // 1. 把所有图片放到 public/images/ 目录
  // 2. 保持当前 /images/xxx.jpg 路径不变
  // 3. 把 calendlyUrl、邮箱、Instagram 改成你的正式信息
  // 4. 推荐部署到 Vercel，域名可绑定 tinycopic.com
  // 5. 如需正式 Next.js 项目，可将此组件放入 app/page.tsx 或 pages/index.tsx
    ...
    <div className="min-h-screen bg-amber-50 text-neutral-900">
      ...
      <section id="services" className="mx-auto max-w-7xl px-6 py-20 lg:px-8">
        <div className="mb-10 flex items-end justify-between gap-6">
          <div>
            <div className="text-sm uppercase tracking-[0.25em] text-amber-700/70">Services</div>
            <h2 className="mt-3 text-3xl font-semibold sm:text-4xl">覆盖摄影工作室的核心高端服务</h2>
          </div>
        ...
        <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          {services.map((item) => (
            <div key={item.title} className="rounded-[28px] border border-amber-200/60 bg-amber-100/50 p-6 transition hover:-translate-y-1 hover:border-white/20 hover:bg-white/[0.05]">
              <div className="inline-flex rounded-full border border-amber-300/20 bg-amber-300/10 px-3 py-1 text-xs font-medium text-amber-200">
                {item.tag}
              </div>
      ...
      <section id="portfolio" className="mx-auto max-w-7xl px-6 py-14 lg:px-8">
        <div className="mb-10 flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <div className="text-sm uppercase tracking-[0.25em] text-amber-700/70">Portfolio</div>
            <h2 className="mt-3 text-3xl font-semibold sm:text-4xl">首页精选作品</h2>
            <p className="mt-4 max-w-2xl text-base leading-7 text-neutral-600">
              以更高级的杂志式编排展示 Tinycopic 的核心风格：梦幻人像、儿童童话、温暖家庭、周岁与新年主题。
            </p>
          </div>
        ...
        <div className="grid auto-rows-[220px] gap-5 md:grid-cols-3 xl:auto-rows-[240px]">
          {portfolio.map((item) => (
            <div
              key={item.title}
              className={`group relative overflow-hidden rounded-[30px] border border-white/70 bg-amber-100/40 shadow-[0_18px_60px_rgba(217,119,6,0.10)] ${item.span}`}
            >
              <img
                src={item.image}
                alt={item.title}
      ...
      <section id="experience" className="mx-auto grid max-w-7xl gap-8 px-6 py-20 lg:grid-cols-[0.85fr_1.15fr] lg:px-8">
        <div className="rounded-[32px] border border-amber-200/60 bg-amber-100/50 p-8">
          <div className="text-sm uppercase tracking-[0.25em] text-amber-700/70">Experience</div>
          <h2 className="mt-3 text-3xl font-semibold">不仅是拍照，更是完整体验设计</h2>
          ...
          <div className="mt-8 rounded-[26px] border border-amber-200/60 bg-amber-100 p-6">
            ...
            <div className="mt-2 text-sm text-neutral-600">Instagram: @tinycopic</div>
        ...
        <div className="grid gap-5 md:grid-cols-2">
          {process.map((item) => (
            <div key={item.step} className="rounded-[28px] border border-amber-200/60 bg-amber-100/50 p-6">
              <div className="text-sm font-semibold text-amber-200/80">{item.step}</div>
      ...
      <section className="mx-auto max-w-7xl px-6 py-10 lg:px-8">
        <div className="mb-10">
          <div className="text-sm uppercase tracking-[0.25em] text-amber-700/70">Testimonials</div>
          <h2 className="mt-3 text-3xl font-semibold sm:text-4xl">让用户愿意下单的信任模块</h2>
        </div>
      ...
      <section className="mx-auto max-w-7xl px-6 py-10 lg:px-8">
        <div className="mb-10 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <div className="text-sm uppercase tracking-[0.25em] text-amber-700/70">Pricing</div>
            <h2 className="mt-3 text-3xl font-semibold sm:text-4xl">清晰套餐，方便客户快速下单</h2>
          </div>
      ...
      <section id="faq" className="mx-auto max-w-7xl px-6 py-20 lg:px-8">
        <div className="mb-10">
          <div className="text-sm uppercase tracking-[0.25em] text-amber-700/70">FAQ</div>
          <h2 className="mt-3 text-3xl font-semibold sm:text-4xl">常见问题</h2>
        </div>
      ...
      <section id="contact" className="mx-auto max-w-7xl px-6 pb-24 lg:px-8">
        <div className="overflow-hidden rounded-[36px] border border-amber-200/60 bg-[linear-gradient(135deg,rgba(251,191,36,0.18),rgba(255,255,255,0.05),rgba(255,255,255,0.02))] p-8 md:p-10">
          <div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr] lg:items-end">
            <div>
              <div className="text-sm uppercase tracking-[0.25em] text-neutral-600">Tinycopic Photography Studio</div>
              ...
              <p className="mt-5 max-w-2xl text-base leading-8 text-neutral-700">
                现在这是一版“摄影行业顶级结构”官网雏形。下一步可以继续接入真实作品、Calendly 链接、品牌故事、价格页、博客、SEO 城市落地页以及客户选片系统。
              </p>
            </div>
            <div className="rounded-[28px] border border-amber-200/60 bg-amber-100 p-6">
              ...
              <div className="mt-2 text-lg font-medium">@tinycopic</div>
